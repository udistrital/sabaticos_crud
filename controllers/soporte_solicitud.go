...
func (c *SoporteSolicitudController) GetOne() {
	...
	v, err := models.GetSoporteSolicitudById(id)
	if err != nil {
		logs.Error(err)
		c.Ctx.Output.SetStatus(400)
...
func (c *SoporteSolicitudController) GetAll() {
	...
	l, err := models.GetAllSoporteSolicitud(query, fields, sortby, order, offset, limit)
	if err != nil {
		logs.Error(err)
		c.Ctx.Output.SetStatus(400)
...
func (c *SoporteSolicitudController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.SoporteSolicitud{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		v.FechaCreacion = time_bogota.TiempoCorreccionFormato(v.FechaCreacion)
		v.FechaModificacion = time_bogota.TiempoBogotaFormato()
		if err := models.UpdateSoporteSolicitudById(&v); err == nil {
...
func (c *SoporteSolicitudController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeleteSoporteSolicitud(id); err == nil {
